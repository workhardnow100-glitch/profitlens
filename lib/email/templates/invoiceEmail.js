// lib/email/templates/invoiceEmail.ts
// PURPOSE:
//   Build the subject, HTML, and text body for an invoice email.
//
// MONEY MODEL (CRITICAL):
//   • invoice amounts (net_amount, tax_amount, gross_amount) are stored in PENCE.
//   • This file MUST convert pence → pounds exactly once.
//   • The previous version incorrectly treated pence as pounds,
//     causing emails to show £1764.00 while PDFs showed £17.64.
//   • This version fixes that by dividing by 100.
//
// POSITION IN PIPELINE:
//   • Called by createInvoiceFromSchedule() after invoice + PDF creation.
//   • Must match PDF formatting exactly.
...
export function buildInvoiceEmail({ invoice, externalClient, senderClient, paymentLinkUrl }) {
  ...
  // ⭐ FIXED: Convert pence → pounds
  const amount = (Number(invoice.gross_amount || 0) / 100).toFixed(2);
