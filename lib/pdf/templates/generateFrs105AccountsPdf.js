...
doc.text(`Closing balance: £${overview?.categories?.assetCost ?? 0}`);
doc.moveDown(1);
...
// Net book value (already net in fixedAssets)
doc.text("Net book value");
doc.text(`At ${formatLongDate(yearEnd)}: £${overview?.categories?.fixedAssets ?? 0}`);
doc.text(`At ${formatLongDate(prevYearEnd)}: £${overviewPrior?.categories?.fixedAssets ?? 0}`);
doc.moveDown(2);
