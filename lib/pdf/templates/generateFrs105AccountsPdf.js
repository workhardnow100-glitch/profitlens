...
export async function generateFrs105AccountsPdf({
  ...
  // Map cockpit overview into balance sheet objects
  const balanceSheetCurrent = {
    currentAssets: overview?.balance_sheet_summary?.current_assets ?? 0,
    creditors: overview?.balance_sheet_summary?.current_liabilities ?? 0,
    netCurrentAssets: overview?.balance_sheet_summary?.net_assets ?? 0,
    totalAssetsLessLiabilities: overview?.balance_sheet_summary?.total_assets_less_current_liabilities ?? 0,
    totalNetAssets: overview?.balance_sheet_summary?.net_assets ?? 0,
    capitalAndReserves: overview?.balance_sheet_summary?.equity ?? 0,
  };

  const balanceSheetPrior = {
    currentAssets: overviewPrior?.balance_sheet_summary?.current_assets ?? 0,
    creditors: overviewPrior?.balance_sheet_summary?.current_liabilities ?? 0,
    netCurrentAssets: overviewPrior?.balance_sheet_summary?.net_assets ?? 0,
    totalAssetsLessLiabilities: overviewPrior?.balance_sheet_summary?.total_assets_less_current_liabilities ?? 0,
    totalNetAssets: overviewPrior?.balance_sheet_summary?.net_assets ?? 0,
    capitalAndReserves: overviewPrior?.balance_sheet_summary?.equity ?? 0,
  };
  ...
  const buffer = await createPdfBuffer((doc) => {
    ...
    const rows = [
      ...
      ["Net assets", balanceSheetCurrent.totalNetAssets, balanceSheetPrior.totalNetAssets],
