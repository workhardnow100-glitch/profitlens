...
const rows = [
  [
    "Fixed assets",
    (overview?.categories?.fixedAssets ?? 0) - Math.abs(overview?.categories?.accumulatedDepreciation ?? 0),
    (overviewPrior?.categories?.fixedAssets ?? 0) - Math.abs(overviewPrior?.categories?.accumulatedDepreciation ?? 0),
  ],
  [
    "Current assets",
...
doc.text(`Closing balance: £${Math.abs(overview?.categories?.accumulatedDepreciation ?? 0)}`);
doc.moveDown(1);

// Net book value (cost - depreciation)
doc.text("Net book value");
doc.text(
  `At ${formatLongDate(yearEnd)}: £${
    (overview?.categories?.fixedAssets ?? 0) - Math.abs(overview?.categories?.accumulatedDepreciation ?? 0)
  }`
);
doc.text(
  `At ${formatLongDate(prevYearEnd)}: £${
    (overviewPrior?.categories?.fixedAssets ?? 0) - Math.abs(overviewPrior?.categories?.accumulatedDepreciation ?? 0)
  }`
);
doc.moveDown(2);
