...
const balanceSheetCurrent = {
  fixedAssets: (overview?.categories?.assetCost ?? 0) - (overview?.categories?.accumulatedDepreciation ?? 0),
  ...
  totalAssetsLessLiabilities: ((overview?.categories?.assetCost ?? 0) - (overview?.categories?.accumulatedDepreciation ?? 0))
    + (((overview?.categories?.bank ?? 0) + (overview?.categories?.receivables ?? 0)) - (overview?.categories?.payables ?? 0)),
  netAssets: overview?.categories?.equity ?? overview?.totals?.total_equity ?? 0,
  capitalAndReserves: overview?.categories?.equity ?? overview?.totals?.total_equity ?? 0,
};

const balanceSheetPrior = {
  fixedAssets: (overviewPrior?.categories?.assetCost ?? 0) - (overviewPrior?.categories?.accumulatedDepreciation ?? 0),
  ...
  totalAssetsLessLiabilities: ((overviewPrior?.categories?.assetCost ?? 0) - (overviewPrior?.categories?.accumulatedDepreciation ?? 0))
    + (((overviewPrior?.categories?.bank ?? 0) + (overviewPrior?.categories?.receivables ?? 0)) - (overviewPrior?.categories?.payables ?? 0)),
  netAssets: overviewPrior?.categories?.equity ?? overviewPrior?.totals?.total_equity ?? 0,
  capitalAndReserves: overviewPrior?.categories?.equity ?? overviewPrior?.totals?.total_equity ?? 0,
};
  ...
  const buffer = await createPdfBuffer((doc) => {
    ...
    const rows = [
      ["Fixed assets",
        ...
        (overviewPrior?.categories?.assetCost ?? 0) - (overviewPrior?.categories?.accumulatedDepreciation ?? 0)
      ],
      ["Current assets",
    ...
    doc.text(`Opening balance as at 01 January ${currentYear}: £${(overviewPrior?.categories?.assetCost ?? 0) - (overviewPrior?.categories?.accumulatedDepreciation ?? 0)}`);
