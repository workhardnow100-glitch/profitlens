...
export async function generateFrs105AccountsPdf({
 ...
 const balanceSheetCurrent = {
  ...
  netAssets: (overview?.totals?.total_assets ?? 0) - (overview?.totals?.total_liabilities ?? 0),   // ✅ fix
  capitalAndReserves: (overview?.totals?.total_assets ?? 0) - (overview?.totals?.total_liabilities ?? 0),   // ✅ fix
};
  ...
  netAssets: (overviewPrior?.totals?.total_assets ?? 0) - (overviewPrior?.totals?.total_liabilities ?? 0),   // ✅ fix
  capitalAndReserves: (overviewPrior?.totals?.total_assets ?? 0) - (overviewPrior?.totals?.total_liabilities ?? 0),   // ✅ fix
};
...
// UPDATED rows: current year depreciation applied, prior year untouched
const rows = [
  [
    "Fixed assets",
    (overview?.categories?.fixedAssets ?? 0) - (overview?.categories?.depreciationCharge ?? 0),
...
[
  ...
  (overview?.totals?.total_assets ?? 0) - (overview?.totals?.total_liabilities ?? 0),   // current year
  overviewPrior?.categories?.equity ?? overviewPrior?.totals?.total_equity ?? 0,       // prior year locked to journals
],
...
doc.text(`At 01 January ${currentYear}: £${overviewPrior?.categories?.assetCost ?? 0}`);
doc.text(`Additions: £${overview?.categories?.assetAdditions ?? 0}`);
doc.text(`Disposals: £${overview?.categories?.assetDisposals ?? 0}`);
doc.text(`At 31 December ${currentYear}: £${overview?.categories?.assetCost ?? 0}`);
doc.moveDown(1);
...
const openingNBV = overviewPrior?.categories?.fixedAssets ?? 0;
const chargeForYear = overview?.categories?.depreciationCharge ?? 0;
const closingNBV = (overview?.categories?.fixedAssets ?? 0) - chargeForYear;

doc.text(`Closing balance as at 31 December ${currentYear}: £${closingNBV}`);
doc.text(`Opening balance as at 01 January ${currentYear}: £${openingNBV}`);
doc.moveDown(2);
