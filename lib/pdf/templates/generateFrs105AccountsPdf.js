...
export async function generateFrs105AccountsPdf({
 ...
 const balanceSheetCurrent = {
  ...
  capitalAndReserves: overview?.totals?.total_equity ?? 0,   // ✅ matches rows
};
...
const balanceSheetPrior = {
  ...
  capitalAndReserves: overviewPrior?.totals?.total_equity ?? 0,   // ✅ matches rows
};
  ...
  const buffer = await createPdfBuffer((doc) => {
    ...
    const rows = [
      ["Fixed assets",
        overview?.categories?.fixedAssets ?? 0,   // NBV
        overviewPrior?.categories?.fixedAssets ?? 0   // NBV
      ],
      ["Current assets",
