...
export async function generateFrs105AccountsPdf({
 ...
 // Balance sheet objects (NBV only)
const balanceSheetCurrent = {
  ...
  netCurrentAssets: (overview?.totals?.current_assets ?? 0) - (overview?.totals?.current_liabilities ?? 0),
  totalAssetsLessLiabilities: (overview?.totals?.total_assets ?? 0) - (overview?.totals?.current_liabilities ?? 0),
...
const balanceSheetPrior = {
  ...
  netCurrentAssets: (overviewPrior?.totals?.current_assets ?? 0) - (overviewPrior?.totals?.current_liabilities ?? 0),
  totalAssetsLessLiabilities: (overviewPrior?.totals?.total_assets ?? 0) - (overviewPrior?.totals?.current_liabilities ?? 0),
...
const buffer = await createPdfBuffer((doc) => {
  ...
  const rows = [
    ...
    ["Creditors: amounts falling due within one year",
      overview?.totals?.current_liabilities ?? 0,
      overviewPrior?.totals?.current_liabilities ?? 0
    ],
    ["Net current assets",
      (overview?.totals?.current_assets ?? 0) - (overview?.totals?.current_liabilities ?? 0),
      (overviewPrior?.totals?.current_assets ?? 0) - (overviewPrior?.totals?.current_liabilities ?? 0)
    ],
    ["Total assets less current liabilities",
      (overview?.totals?.total_assets ?? 0) - (overview?.totals?.current_liabilities ?? 0),
      (overviewPrior?.totals?.total_assets ?? 0) - (overviewPrior?.totals?.current_liabilities ?? 0)
    ],
    ["Net assets",
