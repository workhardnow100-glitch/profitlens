...
doc.text(`At 01 January ${priorYear}: £${overviewPrior?.categories?.assetCost ?? 0}`);
...
doc.text(`At 01 January ${priorYear}: £${overviewPrior?.categories?.accumulatedDepreciation ?? 0}`);
...
doc.text(`Opening balance as at 01 January ${priorYear}: £${overviewPrior?.categories?.fixedAssets ?? 0}`);
doc.moveDown(2);
