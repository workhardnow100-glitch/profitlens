...
export async function generateFrs105AccountsPdf({
  ...
  const buffer = await createPdfBuffer((doc) => {
    ...
    const rows = [
      [
        "Fixed assets",
        (overview?.categories?.fixedAssets ?? 0) - (overview?.categories?.accumulatedDepreciation ?? 0),
        (overviewPrior?.categories?.fixedAssets ?? 0) - (overviewPrior?.categories?.accumulatedDepreciation ?? 0),
      ],
      [
        "Current assets",
        (overview?.categories?.bank ?? 0) + (overview?.categories?.receivables ?? 0),
        (overviewPrior?.categories?.bank ?? 0) + (overviewPrior?.categories?.receivables ?? 0),
      ],
      ...
      [
        "Net current assets",
        (overview?.totals?.totalCurrentAssets ?? 0) - (overview?.totals?.totalCurrentLiabilities ?? 0),
        (overviewPrior?.totals?.totalCurrentAssets ?? 0) - (overviewPrior?.totals?.totalCurrentLiabilities ?? 0),
      ],
      [
        "Total assets less current liabilities",
        (overview?.totals?.totalAssets ?? 0) - (overview?.totals?.totalCurrentLiabilities ?? 0),
        (overviewPrior?.totals?.totalAssets ?? 0) - (overviewPrior?.totals?.totalCurrentLiabilities ?? 0),
      ],
      [
        "Net assets",
        (overview?.totals?.totalAssets ?? 0) - (overview?.totals?.totalLiabilities ?? 0),
        (overviewPrior?.totals?.totalAssets ?? 0) - (overviewPrior?.totals?.totalLiabilities ?? 0),
      ],
      ["Capital and reserves", overview?.categories?.equity ?? 0, overviewPrior?.categories?.equity ?? 0],
    ];
    ...
    // NEW Tangible Fixed Assets note
    doc.fontSize(14).text("2. Tangible fixed assets", { underline: true });
    doc.moveDown(1);
    doc.fontSize(12).text("Cost or valuation");
    doc.text(`Closing balance: £${overview?.categories?.fixedAssets ?? 0}`);
    doc.moveDown(1);
    doc.text("Depreciation");
    doc.text(`Closing balance: £${overview?.categories?.accumulatedDepreciation ?? 0}`);
    doc.moveDown(1);
    doc.text("Net book value");
    doc.text(`At ${formatLongDate(yearEnd)}: £${(overview?.categories?.fixedAssets ?? 0) - (overview?.categories?.accumulatedDepreciation ?? 0)}`);
    doc.text(`At ${formatLongDate(prevYearEnd)}: £${(overviewPrior?.categories?.fixedAssets ?? 0) - (overviewPrior?.categories?.accumulatedDepreciation ?? 0)}`);
    doc.moveDown(2);
...
// Use the builder's category instead of hard-coded account codes
