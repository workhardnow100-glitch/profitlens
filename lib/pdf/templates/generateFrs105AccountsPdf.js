...
export async function generateFrs105AccountsPdf({
  ...
  const buffer = await createPdfBuffer((doc) => {
    ...
    y += 20;
      ...
      [
        "Fixed assets",
        (overview?.accounts?.["1012"]?.balance ?? 0) -
          (overview?.accounts?.["1099"]?.balance ?? 0),
        (overviewPrior?.accounts?.["1012"]?.balance ?? 0) -
          (overviewPrior?.accounts?.["1099"]?.balance ?? 0),
      ],
      [
        "Current assets",
        (overview?.accounts?.["1100"]?.balance ?? 0),
        (overviewPrior?.accounts?.["1100"]?.balance ?? 0),
      ],
      [
        "Creditors: amounts falling due within one year",
        (overview?.accounts?.["2100"]?.balance ?? 0),
        (overviewPrior?.accounts?.["2100"]?.balance ?? 0),
      ],
      [
        "Net assets",
        ((overview?.accounts?.["1012"]?.balance ?? 0) -
