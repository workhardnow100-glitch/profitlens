  ...
  const buffer = await createPdfBuffer((doc) => {
    ...
    contents.forEach((item) => {
      ...
      const dotsCount = Math.max(
        0,
        Math.floor((lineWidth - labelWidth - pageWidth) / 3)
      );
      const dots = ".".repeat(dotsCount);
      doc.text(`${item.label} ${dots} ${item.page}`);
    });
    ...
    doc.fontSize(12);
    doc.text(`${currentYear} (£)    ${priorYear} (£)`, { align: "right" });
    doc.moveDown(1);
