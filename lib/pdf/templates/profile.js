...
export async function generateProfilePdf({
  ...
  const buffer = await createPdfBuffer((doc) => {
    ...
    doc.save();
    doc.lineWidth(2);
    doc.strokeColor("#1e293b"); // slate-800
    ...
    //
    // ─────────────────────────────────────────────
    // PERSONAL + BUSINESS DETAILS (SIDE BY SIDE)
    // ─────────────────────────────────────────────
    //
    const leftX = 50;
    const rightX = 300;
    ...
    doc.fontSize(12).fillColor("black");
    doc.text(
      `Total Income: £${Number(hmrcBreakdown?.totalIncome || 0).toFixed(2)}`
    );
    doc.text(
      `Allowable Expenses: £${Number(
        hmrcBreakdown?.allowable || 0
      ).toFixed(2)}`
    );
    doc.text(
      `Disallowable Expenses: £${Number(
        hmrcBreakdown?.disallowable || 0
      ).toFixed(2)}`
    );
    doc.text(
      `Net Profit: £${Number(hmrcBreakdown?.netProfit || 0).toFixed(2)}`
    );
    doc.text(
      `Tax Rate: ${(hmrcBreakdown?.soleTraderTaxRate || 0) * 100}%`
    );
    doc.text(
      `Tax Owed: £${Number(hmrcBreakdown?.soleTraderOwed || 0).toFixed(2)}`
    );
    doc.moveDown(1.5);
    ...
    doc.text(
      `Corporation Tax Rate: ${
        (hmrcBreakdown?.limitedCompanyTaxRate || 0) * 100
      }%`
    );
    doc.text(
      `Corporation Tax Owed: £${Number(
        hmrcBreakdown?.limitedCompanyOwed || 0
      ).toFixed(2)}`
    );
    doc.moveDown(1.5);
