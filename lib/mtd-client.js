...
export async function createClient(clientId) {
  ...
  return {
    ...
    async getVATLiabilities() {
      if (!vrn) throw new Error("Client VAT number missing");
      return hmrcGet(`/organisations/vat/${vrn}/liabilities`);
    },

    // ⭐ VAT Payments
    async getVATPayments() {
      if (!vrn) throw new Error("Client VAT number missing");
      return hmrcGet(`/organisations/vat/${vrn}/payments`);
    },

    // ⭐ VAT Periods (HMRC uses obligations as periods)
    async getVATPeriods() {
      if (!vrn) throw new Error("Client VAT number missing");
      return hmrcGet(`/organisations/vat/${vrn}/obligations`);
    },

    // ⭐ VAT MTD Connection Status
    async getVATConnectionStatus() {
      if (!vrn) throw new Error("Client VAT number missing");

      try {
        const obligations = await hmrcGet(`/organisations/vat/${vrn}/obligations`);
    ...
    // ---------------------------
    // CIS MTD (Contractor + Subcontractor)
    // ---------------------------
    // CIS MTD connection / health
    async getCISConnectionStatus() {
      if (!utr) throw new Error("Client UTR missing for CIS");

      try {
        // Use obligations as a lightweight health check
        const obligations = await hmrcGet(`/cis/${utr}/obligations`);
    ...
    // CIS obligations (periods)
    async getCISObligations() {
      if (!utr) throw new Error("Client UTR missing for CIS");
      return hmrcGet(`/cis/${utr}/obligations`);
    },

    // CIS returns (history)
    async getCISReturns() {
      if (!utr) throw new Error("Client UTR missing for CIS");
      return hmrcGet(`/cis/${utr}/returns`);
    },

    // CIS deductions (HMRC view: deducted + suffered)
    async getCISDeductions() {
      if (!utr) throw new Error("Client UTR missing for CIS");
      return hmrcGet(`/cis/${utr}/deductions`);
    },

    // CIS receipt lookup by submissionId
    async getCISReceipt(submissionId) {
      if (!utr) throw new Error("Client UTR missing for CIS");
      if (!submissionId) throw new Error("Missing submissionId for CIS receipt");
      return hmrcGet(`/cis/${utr}/returns/${encodeURIComponent(submissionId)}`);
    },

    // CIS subcontractor verification
    async verifySubcontractor(subUtr) {
      if (!utr) throw new Error("Client UTR missing for CIS (contractor)");
      if (!subUtr) throw new Error("Missing subcontractor UTR");

      // HMRC CIS verification endpoint shape may vary — treat this as abstracted
      return hmrcPost(`/cis/${utr}/verifications`, {
        subcontractor: {
          utr: subUtr
        }
      });
    ...
    // ---------------------------
    async getCTConnectionStatus() {
      if (!utr) throw new Error("Client UTR missing for Corporation Tax");

      try {
        // Use obligations as a lightweight health check, same pattern as VAT/CIS
    ...
    // CT obligations (open/fulfilled filing obligations)
    async getCTObligations() {
      if (!utr) throw new Error("Client UTR missing for Corporation Tax");
      return hmrcGet(
        `/organisations/corporation-tax/${utr}/obligations`
      );
    ...
    // CT returns (submitted CT returns)
    async getCTReturns() {
      if (!utr) throw new Error("Client UTR missing for Corporation Tax");
      return hmrcGet(
        `/organisations/corporation-tax/${utr}/returns`
      );
    },

    // CT liabilities (amounts due)
    async getCTLiabilities() {
      if (!utr) throw new Error("Client UTR missing for Corporation Tax");
      return hmrcGet(
        `/organisations/corporation-tax/${utr}/liabilities`
      );
    },

    // CT payments (payments made to HMRC)
    async getCTPayments() {
      if (!utr) throw new Error("Client UTR missing for Corporation Tax");
      return hmrcGet(
        `/organisations/corporation-tax/${utr}/payments`
      );
    }
  };
}
