...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      // 🔄 Return updated profile data immediately after update
      req.method = "GET";
    }
    ...
    if (req.method === "GET") {
      ...
      // Fetch global HMRC categories only
      ...
      transactions.forEach(tx => {
        ...
        // Skip excluded categories (including Disposal of Fixed Asset)
        if (cat?.is_excluded) return;

        if (tx.amount > 0) {
          totalIncome += tx.amount;
        } else {
          // Allowable expenses are still string‑matched for now
