...
export default async function handler(req, res) {
  ...
  try {
    // 1️⃣ Optional: check for incomplete transactions
    const { data: incompleteTx, error: checkError } = await supabase
      ...
      .lte("date", periodEnd)
      .is("amount", null); // check for missing amounts

    if (checkError) throw new Error(checkError.message);

    if (incompleteTx.length > 0) {
      return res.status(400).json({
        error: "Cannot submit. Some transactions are incomplete."
      });
    }

    // 2️⃣ Lock all transactions for the period
