...
export default async function handler(req, res) {
  ...
  if (!internalBypass && session.user.role === "accountant" && clientId !== actingClientId) {
    return res.status(403).json({ error: "Accountants cannot view VAT summaries for unauthorized clients" });
  }
  ...
  try {
    ...
    const { data: transactions, error: txError } = await supabase
      .from("transactions")
      .select("id,date,description,amount,type,business_category,vat_rate,vat_amount,tax_locked")
      .eq("client_id", clientId)
    ...
    const isLockedByTx = vatTransactions.length > 0 && vatTransactions.every(tx => tx.tax_locked === true);
    ...
    for (const tx of vatTransactions) {
      ...
      if (classification==="income"){box6+=netAbs;if(vatAbs>0)box1+=vatAbs;}
      if (classification==="expense"){box7+=netAbs;if(vatAbs>0)box4+=vatAbs;}
    }
    let box3=box1+box2;
    let box5=box3-box4;
    ...
    if (vatPeriodId) {
      const { data: adjData, error: adjError } = await supabase
        .from("vat_adjustments").select("*")
        .eq("client_id",clientId).eq("vat_period_id",vatPeriodId);
    ...
    return res.status(200).json({
      ...
      boxes:{
        box1:+box1.toFixed(2),box2:+box2.toFixed(2),box3:+box3.toFixed(2),
        box4:+box4.toFixed(2),box5:+box5.toFixed(2),box6:+box6.toFixed(2),
