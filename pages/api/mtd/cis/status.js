...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ⭐ AUDIT LOG — Accountant viewing CIS MTD status
    if (role === "ACCOUNTANT") {
      await supabaseAdmin.from("audit").insert([
        {
          client_id: clientId,
          actor_email: session.user.email,
          action: "ACCOUNTANT_VIEW_CIS_MTD_STATUS",
          details: "Viewed CIS MTD connection status",
          timestamp: new Date().toISOString(),
        },
      ]);
    }

    // ⭐ Fetch CIS MTD connection status
    const status = await mtd.getCISConnectionStatus();
