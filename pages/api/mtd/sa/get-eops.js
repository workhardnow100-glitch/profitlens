...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ⭐ AUDIT LOG — Accountant viewing SA MTD EOPS
    if (role === "ACCOUNTANT") {
      await supabaseAdmin.from("audit").insert([
        {
          client_id: clientId,
          actor_email: session.user.email,
          action: "ACCOUNTANT_VIEW_MTD_SA_EOPS",
          details: "Viewed SA MTD EOPS",
          timestamp: new Date().toISOString(),
        },
      ]);
    }

    // ⭐ Fetch EOPS from HMRC
    const eops = await mtd.getEOPS();
    ...
    console.error("SA MTD get EOPS error:", err);
